...
# SERIAL
# =========================
SERIAL_PORT = "/dev/ttyUSB0"     # change if needed: /dev/ttyACM0
BAUD_RATE = 115200
...
def send_with_ack(ser, cmd: bytes, timeout=0.7, retries=3) -> bool:
    """
    cmd: b"L" or b"R"
    Arduino replies: ACK:L or ACK:R
    """
    expected = b"ACK:" + cmd
    for _ in range(retries):
        # Clear old printed status so we don't confuse it with ACK
        try:
...
def pick_decision(result, names, roi):
    ...
    - prefers recyclable if any recyclable is present
    """
...
def main():
    ...
    try:
        ...
        while True:
            # Read Arduino lines (bin status + ACK prints)
            ...
            # send once per object appearance
            if (not latched) and stable >= STABLE_FRAMES_REQUIRED and (now - last_sent_time) >= COOLDOWN_SECONDS:
                latched = True
                last_sent_time = now
